...
export const createMap = async () => {
    ...
    let visibleLayers = new Set(['Placement_map']); // FIXME: re-add placement?
    ...
    //
    //     console.log(Object.keys(map.groups));
    ...
    map.on('hashmetainit', function (initState) {
        ...
        layers.filter((name) => name in extraLayers).forEach((layerName) => visibleLayers.add(layerName));
    ...
    map.on('overlayremove', function (eventLayer) {
        if (eventLayer.name === 'Check_Power' || eventLayer.name === 'Check_Sound' || eventLayer.name === 'Check_Clean')
